...
BEGIN
	...
	SELECT COALESCE(dca.CompanyName,'') AS ShipTo,
		...
		WHERE  od.ShipmentDate >= @WeekStartDate AND od.ShipmentDate <= @WeekEndDate AND  os.ID NOT IN (18,22,23,24,28,31) AND (os.Name  != 'Indico Hold' AND os.Name != 'Indiman Hold' 
	...
	SELECT  CONVERT(nvarchar(4),DATEPART(YEAR,@ETD)) + '/' + CONVERT(nvarchar(2),@WeekNumber) AS [Week],
		...
		FROM [dbo].[Order] o
			...
			LEFT OUTER JOIN [dbo].[OrderDetailStatus] ods
				ON od.[Status] = ods.ID
			WHERE os.ID NOT IN (18,22,23,24,28,31)  AND dca.CompanyName = @P_ShipTo AND od.ShipmentDate = @ETD AND odq.Qty>0-- AND  os.Name = @P_Status
...
DELETE [dbo].[WeeklyProductionCapacityDetails]
...
WHERE DATEPART(YEAR,wpc.WeekendDate) > 2017
GO

DELETE [dbo].WeeklyProductionCapacity
	FROM [dbo].[WeeklyProductionCapacity] wpc
WHERE DATEPART(YEAR,wpc.WeekendDate) > 2017
...
-- Loading Patterns for Web service --

/****** Object:  StoredProcedure [dbo].[SPC_GetPatternDetails]    Script Date: 8/10/2016 7:55:27 PM ******/
