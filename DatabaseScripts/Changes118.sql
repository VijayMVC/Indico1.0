...
--/****** Object:  StoredProcedure [dbo].[SPC_GetOrderDetailIndicoPrice]    Script Date: 11/19/2014 12:34:57 ******/
...
--					ELSE 
--						BEGIN
--							SET @Price = (SELECT ISNULL(((ISNULL((plc.[IndimanCost] - 2) ,0.00)) / (1 - ((SELECT dpm.[Markup] 
--																														    FROM [dbo].[DistributorPriceMarkup] dpm 
--																															WHERE  ((@HasDistributor = 0 AND dpm.[Distributor] IS NULL) OR dpm.[Distributor] = @Distributor) 
...
----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***----**-**-***--
--/****** Object:  StoredProcedure [dbo].[SPC_GetIndimanPriceListData]    Script Date: 11/19/2014 13:11:41 ******/
...
--										AND dplc.PriceLevelCost = plc.ID), 0.00) = 0.00)
--					THEN (ISNULL(((100 * (plc.[IndimanCost] - 2)) / (100 - dpm.[Markup])), 0.00))
--					ELSE (SELECT CASE
--							WHEN (ISNULL((SELECT dplc.IndicoCost 
--										  FROM	 DistributorPriceLevelCost dplc WITH (NOLOCK)
--										  WHERE	 ((@P_Distributor = 0 AND dplc.[Distributor] IS NULL) 
...
--																			  OR dplc.[Distributor] = @P_Distributor)
--																			  AND dplc.PriceTerm = 1
--																			  AND dplc.PriceLevelCost = plc.ID) - 2) / (1 - ((100 - ((plc.[IndimanCost] * 100)/(SELECT dplc.IndicoCost FROM DistributorPriceLevelCost dplc WITH (NOLOCK)
--																																													   WHERE ((@P_Distributor = 0 AND dplc.[Distributor] IS NULL) 
--																																													   OR dplc.[Distributor] = @P_Distributor)
--																																													   AND dplc.PriceTerm = 1
...
UPDATE [dbo].[OrderStatus] SET [Name] = 'Submitted by Distributor' WHERE [Name] = 'Distributor Submitted'
GO
