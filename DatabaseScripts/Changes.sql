...
BEGIN
	...
	SELECT @ClientDistributor AS CliendDis

	SELECT * FROM [dbo].[Client] WHERE ID = @Client

	IF  (@ClientDistributor != @P_Distributor)
	BEGIN
		INSERT INTO [dbo].[Client] (Name,Distributor,[Description],FOCPenalty)
			SELECT TOP 1 Name,@P_Distributor,[Description],FOCPenalty FROM [dbo].[Client] WHERE ID = @Client
		SET @Client = SCOPE_IDENTITY()
		SELECT @Client AS InsertedClient
		INSERT INTO [dbo].[JobName] (Name,[Address],City,[State],PostalCode,Country,Phone,Email,Creator,CreatedDate,Modifier,ModifiedDate,Client)
			SELECT Name,[Address],City,[State],PostalCode,Country,Phone,Email,Creator,CreatedDate,Modifier,ModifiedDate,@Client AS Client FROM [dbo].[JobName] WHERE ID = @P_JobName
	END

	--SET @ClientName = (SELECT TOP 1 cl.Name FROM [dbo].[Client] cl 
	--					    WHERE cl.ID = @Client)

	--IF NOT EXISTS (SELECT di.ID FROM [dbo].[Company] di
	--						INNER JOIN  [dbo].[Client] cl
	--							ON cl.Distributor = di.ID
	--		   WHERE cl.Name = @ClientName AND di.ID = @P_Distributor)
	--BEGIN
	--	INSERT INTO [dbo].[Client] (Name,Distributor,[Description],FOCPenalty)
	--		SELECT TOP 1 Name,@P_Distributor,[Description],FOCPenalty FROM [dbo].[Client]
	--	SET @Client = SCOPE_IDENTITY()
	--	INSERT INTO [dbo].[JobName] (Name,[Address],City,[State],PostalCode,Country,Phone,Email,Creator,CreatedDate,Modifier,ModifiedDate,Client)
	--		SELECT Name,[Address],City,[State],PostalCode,Country,Phone,Email,Creator,CreatedDate,Modifier,ModifiedDate,@Client AS Client FROM [dbo].[JobName] WHERE ID = @P_JobName
	--END
	--ELSE 
	--BEGIN
	--	SET @Client = (SELECT TOP 1 cl.ID FROM [dbo].[Client] cl WHERE cl.Name = @ClientName)
	--	INSERT INTO [dbo].[JobName] (Name,[Address],City,[State],PostalCode,Country,Phone,Email,Creator,CreatedDate,Modifier,ModifiedDate,Client)
	--		SELECT Name,[Address],City,[State],PostalCode,Country,Phone,Email,Creator,CreatedDate,Modifier,ModifiedDate,@Client AS Client FROM [dbo].[JobName] WHERE ID = @P_JobName
	--END
END
...
CREATE PROC [dbo].[TransferVisualLayout]
	@P_VisualLayout int,
	@P_JobName int
AS
BEGIN
