...
-- Transfer BLACKCHROME - WAAFL to BLACKCHROME - Michael McCarthy

DECLARE @WAAFL int
DECLARE @Michael int

SET @WAAFL = (SELECT TOP 1 ID FROM [dbo].[Company] WHERE Name LIKE 'BLACKCHROME - WAAFL')
SET @Michael = (SELECT TOP 1 ID FROM [dbo].[Company] where Name LIKE 'BLACKCHROME - Michael McCarthy')


IF @Michael > 0 AND @WAAFL > 0
BEGIN
	UPDATE dca
	SET dca.Distributor = @Michael
	FROM [dbo].[DistributorClientAddress] dca
		WHERE dca.Distributor = @WAAFL

	UPDATE dl
	SET dl.Distributor = @Michael
	FROM [dbo].[DistributorLabel] dl
	WHERE dl.Distributor = @WAAFL

	UPDATE c
	SET c.Distributor = @Michael
	FROM [dbo].[Client] c
		WHERE c.Distributor = @WAAFL
END
