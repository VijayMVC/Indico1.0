...
--**-**-**--**-**-**--**-**-**--**-**-** ADD New two columns to the OrderDetil --**-**-**--**-**-**--**-**-**

ALTER TABLE [dbo].[OrderDetail]
ADD [EditedPrice] [decimal] (8,2) 
GO

ALTER TABLE [dbo].[OrderDetail]
ADD [EditedPriceRemarks] [nvarchar] (255) 
GO

--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**--**-**-**

--**-**-**--**-**-**--**-**-**--**-**-** Create new SPC Delete Order --**-**-**--**-**-**--**-**-**--**-**-**--**-**-**


--****** Object:  StoredProcedure [dbo].[SPC_DeleteOrder]    Script Date: 10/04/2013 12:13:01 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SPC_DeleteOrder]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SPC_DeleteOrder]
GO
...
CREATE PROCEDURE [dbo].[SPC_DeleteOrder]
(
	@P_ID int	
)
AS 
...
BEGIN
	...
	DECLARE @RetVal int
		...
		BEGIN TRY
				...
				FROM [dbo].[PackingListCartonItem] plci
					JOIN [dbo].[PackingList] pl
						ON plci.[PackingList] = pl.[ID]
					JOIN [dbo].[OrderDetail] od
						ON pl.[OrderDetail] = od.[ID] 
					JOIN [dbo].[Order] o
						ON od.[Order] = o.[ID]
...
(
	...
	WHERE od.[Order] = @P_Order	
		...
		SET @HasDistributor = (SELECT COUNT(*) FROM [dbo].[DistributorPriceLevelCost] WHERE [Distributor] = @Distributor)
			...
			SET	@Price = (SELECT ISNULL(dplc.[IndicoCost], ISNULL(((plc.[IndimanCost] * 100) / (100 - (SELECT dpm.[Markup] 
																			  FROM [dbo].[DistributorPriceMarkup] dpm 
																			  WHERE ((@HasDistributor =0 AND dplc.[Distributor] IS NULL) OR dplc.[Distributor] = @Distributor ) 
...
/****** Object:  View [dbo].[ReturnOrderDetailsIndicoPriceView]    Script Date: 10/07/2013 17:56:04 ******/
