...
IF EXISTS ( SELECT * 
            FROM   sysobjects 
            WHERE  id = object_id(N'[dbo].[SPC_GetShipmentKeys]') 
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
...
BEGIN
 ...
 SELECT 
	...
	dca.CompanyName AS ShipToCompanyName,
	dca.ID AS ShipTo,
	od.ShipmentDate,
...
IF EXISTS ( SELECT * 
            FROM   sysobjects 
            WHERE  id = object_id(N'[dbo].[SPC_GetJKDetailInvoiceInfo]') 
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
    DROP PROCEDURE [dbo].[SPC_GetJKDetailInvoiceInfo]
END
GO

CREATE PROCEDURE [dbo].[SPC_GetJKDetailInvoiceInfo] (	
	@P_InvoiceId int
)	
AS 
BEGIN
	...
	SELECT DISTINCT
			...
			o.Client AS Client,
			j.Name AS dbo_JobName_Name,
			inv.FactoryPrice AS QuotedFOBCost,
			inv.OtherCharges AS OC,
			(inv.FactoryPrice + inv.OtherCharges) AS fp,
			qi.Qty AS SumOfQty,
			((inv.FactoryPrice + inv.OtherCharges) * qi.Qty) AS Val, 
			os.ID AS [Status],
			os.[Description] AS [Description],
