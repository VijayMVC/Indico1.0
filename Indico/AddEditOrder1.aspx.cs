...
namespace Indico
{
    public partial class AddEditOrder1 : IndicoPage
    {
        #region enums
        ...
        protected int OrderID
        {
            get
            {
                ...
                if ((ViewState["OrderID_" + LoggedUser.ID.ToString()] != null) && (orderID == 0))
                {
                    orderID = int.Parse(ViewState["OrderID_" + LoggedUser.ID.ToString()].ToString());
                }
        ...
        private string SortExpression
        {
            get
            {
                string sort = (string)Session["ProductSortExpression"];
            ...
            set
            {
                Session["ProductSortExpression"] = value;
            }
        }
        ...
        protected void dgOrderItems_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (item.ItemIndex > -1 && item.DataItem is ReturnOrderDetailsIndicoPriceViewBO)
            {
                ...
                lblOrderType.Text = objOrderDetail.OrderType;

                Literal lblVLNumber = (Literal)item.FindControl("lblVLNumber");
                lblVLNumber.Text = (string.IsNullOrEmpty(objOrderDetail.VisualLayout) ? string.Empty : objOrderDetail.VisualLayout + " / ")
                    + objOrderDetail.Pattern + " / " + objOrderDetail.Fabric;
                ...
                ancVLImage.HRef = IndicoPage.GetVLImagePath(int.Parse(objOrderDetail.VisualLayoutID.ToString()));
                ...
                objOrderDetailQty.OrderDetail = (int)objOrderDetail.OrderDetail;

                Repeater rptSizeQtyView = (Repeater)item.FindControl("rptSizeQtyView");
                rptSizeQtyView.DataSource = objOrderDetailQty.SearchObjects();
                rptSizeQtyView.DataBind();

                Literal lblOdNotes = (Literal)item.FindControl("lblOdNotes");
                lblOdNotes.Text = objOrderDetail.VisualLayoutNotes;
                ...
                string NNFilePath = GetNameNumberFilePath((int)objOrderDetail.OrderDetail);
                ...
                linkEdit.Attributes.Add("qid", objOrderDetail.OrderDetail.ToString());

                OrderDetailBO objOD = new OrderDetailBO();
                objOD.ID = (int)objOrderDetail.OrderDetail;
                objOD.GetObject();
                ...
                litPocket.Text = ((objOD.objVisualLayout.PocketType ?? 0) > 0) ? objOD.objVisualLayout.objPocketType.Name : string.Empty;

                Literal lblQty = (Literal)item.FindControl("lblQty");
                lblQty.Text = objOrderDetail.Quantity.ToString();

                Literal lblSurcharge = (Literal)item.FindControl("lblSurcharge");
                lblSurcharge.Text = objOD.Surcharge.ToString();
                ...
                txtEditedPrice.Text = "$" + ((objOrderDetail.EditedPrice != null && objOrderDetail.EditedPrice != 0) ? Convert.ToDecimal(objOrderDetail.EditedPrice.ToString()).ToString("0.00")
                    : "0.00");
                ...
                bool isDeleteEnabled = (this.LoggedUser.IsDirectSalesPerson && currentStatus == OrderStatus.New)
                ...
                linkDelete.Attributes.Add("qid", objOrderDetail.OrderDetail.ToString());
                ...
                libtnSaveRemarks.Attributes.Add("qid", objOrderDetail.OrderDetail.ToString());

                HyperLink linkCancelDetail = (HyperLink)item.FindControl("linkCancelDetail");
                linkCancelDetail.Attributes.Add("qid", objOrderDetail.OrderDetail.ToString());
        ...
        protected void linkEdit_Click(object sender, EventArgs e)
        {
            ...
            if (IsNotRefreshed)
            {
                try
                {
                    ...
                    hdnSelectedID.Value = objOrderDetail.ID.ToString();

                    ddlSizes.Visible = true;

                    OrderDetailQtyBO objODQ = new OrderDetailQtyBO();
                    objODQ.OrderDetail = objOrderDetail.ID;
                    List<OrderDetailQtyBO> lstExsistingSizes = objODQ.SearchObjects().ToList();

                    SizeBO objSizeBo = new SizeBO();
                    objSizeBo.SizeSet = objOrderDetail.objPattern.SizeSet;
                    ...
                    {
                        if (item.Selected)
                            ddlOrderType.Items.FindByValue(item.Value).Selected = false;
                    }

                    foreach (ListItem item in ddlLabel.Items)
                    {
                        if (item.Selected)
                            ddlLabel.Items.FindByValue(item.Value).Selected = false;
                    }

                    //foreach (ListItem item in ddlVlNumber.Items)
                    //{
                    //    if (item.Selected)
                    //        ddlVlNumber.Items.FindByValue(item.Value).Selected = false;
                    //}
                    ...
                    if (!string.IsNullOrEmpty(objOrderDetail.NameAndNumbersFilePath))
                    {
                        ...
                        string NameAndNumberFileIconPath = GetNameNumberFileIconPath(objOrderDetail.ID);
                        ...
                        //dvFilePreview.Attributes.Add("style", "display:block;");
                        uploadImage.ImageUrl = NameAndNumberFileIconPath;
                        lifileUploder.Visible = false;
                        hdnDeleteFile.Value = objOrderDetail.ID.ToString();
                    ...
                    else
                    {
                        ...
                        linkDelete.Visible = false; ddlOrderType.Items.FindByValue(objOrderDetail.OrderType.ToString()).Selected = true;
                    }

                    ddlOrderType.Items.FindByValue(objOrderDetail.OrderType.ToString()).Selected = true;
                    ddlLabel.Items.FindByValue(objOrderDetail.Label.ToString()).Selected = true;
                    chkPhotoApprovalRequired.Checked = (bool)objOrderDetail.PhotoApprovalReq;
                    chkBrandingKit.Checked = (bool)objOrderDetail.IsBrandingKit;
                    chkLockerPatch.Checked = (bool)objOrderDetail.IsLockerPatch;
                    ...
                    txtPromoCode.Text = objOrderDetail.PromoCode;
                    txtShipmentDate.Text = objOrderDetail.ShipmentDate.ToString("dd MMM yyyy");
                    txtPriceComments.Text = objOrderDetail.EditedPriceRemarks;
                    txtFactoryDescription.Text = objOrderDetail.FactoryInstructions;
                    ...
                    {
                        objOrderDetail.Reservation = int.Parse(RadComboReservations.SelectedValue);
                    }

                    if (objOrderDetail.VisualLayout.HasValue && objOrderDetail.VisualLayout > 0)
                    {
                        PopulateVisualLayouts(0);

                        ddlVlNumber.ClearSelection();
                        ddlVlNumber.Items.FindByValue(objOrderDetail.VisualLayout.ToString()).Selected = true;

                        VisualLayoutBO objVisualLayout = new VisualLayoutBO();
                        objVisualLayout.ID = objOrderDetail.VisualLayout.Value;
                        objVisualLayout.GetObject();

                        if (string.IsNullOrEmpty(objOrderDetail.VisualLayoutNotes))
                        {
                            txtOdNotes.Text = objVisualLayout.Description;
                        }
                        else
                        {
                            txtOdNotes.Text = objOrderDetail.VisualLayoutNotes;
                        }

                        txtUnit.Text = (objVisualLayout.objPattern.Unit != null && objVisualLayout.objPattern.Unit > 0) ? objVisualLayout.objPattern.objUnit.Name : string.Empty;
                        PopulateVlImage(objVisualLayout.ID);
                    ...
                    {
                        VisualLayoutBO objVisualLayout = new VisualLayoutBO();
                        objVisualLayout.Client = objOrderDetail.objOrder.Client;
                        objVisualLayout.Pattern = objOrderDetail.Pattern;
                        objVisualLayout.FabricCode = objOrderDetail.FabricCode;

                        List<VisualLayoutBO> lstVisualLayout = objVisualLayout.SearchObjects().ToList();
                        ...
                        ddlPattern.Items.FindByValue(objOrderDetail.Pattern.ToString()).Selected = true;

                        this.PopulateFabrics(objOrderDetail.Pattern);

                        ddlFabric.ClearSelection();

                        if (ddlFabric.Items.FindByValue(objOrderDetail.FabricCode.ToString()) != null)
                        {
                            ddlFabric.Items.FindByValue(objOrderDetail.FabricCode.ToString()).Selected = true;
                        }
                    ...
                    objPattern.ID = objOrderDetail.Pattern;
                    ...
                    /* else if (objOrderDetail.ArtWork.HasValue && objOrderDetail.ArtWork > 0)
                     {
                         //ddlArtWork.ClearSelection();
                         //ddlArtWork.Items.FindByValue(objOrderDetail.ArtWork.ToString()).Selected = true;

                         ArtWorkBO objArtWork = new ArtWorkBO();
                         objArtWork.ID = objOrderDetail.ArtWork.Value;
                         objArtWork.GetObject();
                    ...
                    PopulateReservations(objOrderDetail.VisualLayout ?? 0, objOrderDetail.ArtWork ?? 0, true);

                    if (objOrderDetail.Reservation != null && objOrderDetail.Reservation > 0)
                    {
                        RadComboReservations.Items.FindItemByValue(objOrderDetail.Reservation.ToString()).Selected = true;
                        PopulateReservationsDetails();
                    }

                    txtIsNew.Text = (objOrderDetail.IsRepeat == true) ? "Repeat" : "New";
                    rbNaNuYes.Checked = (objOrderDetail.IsRequiredNamesNumbers != null && objOrderDetail.IsRequiredNamesNumbers == true) ? true : false;
                    rbNaNuNo.Checked = (objOrderDetail.IsRequiredNamesNumbers == null || !objOrderDetail.IsRequiredNamesNumbers == true) ? true : false;
                    NameNumberFilePath = objOrderDetail.NameAndNumbersFilePath;
                    ...
                    /*(objOrderDetail.objOrder.objCreator.IsDistributor == false);*/
                    ddlFabric.Enabled = ddlPattern.Enabled = false;
                    //ddlArtWork.Enabled = false;
                    PopulatePrice(objOrderDetail.Pattern, objOrderDetail.FabricCode, lstODQty.Sum(v => v.Qty), objOrderDetail.Surcharge, objOrderDetail.EditedPrice ?? 0);
                }
                catch (Exception ex)
                {
        ...
        protected void btnAddOrder_Click(object sender, EventArgs e)
        {
            ...
            if (IsNotRefreshed)
            {
                ...
                if (rbCourier.Checked && ddlCourierAddress.SelectedIndex < 1)
                {
                    cv = new CustomValidator();
                    cv.IsValid = false;
                    cv.ValidationGroup = "valGrpOrderHeader";
                ...
                if (!DateTime.TryParse(txtShipmentDate.Text, out tempDateTime))
                {
                    cv = new CustomValidator();
                    cv.IsValid = false;
                    cv.ValidationGroup = "valGrpOrderHeader";
                    cv.ErrorMessage = "Shipment Date is required";
                    Page.Validators.Add(cv);
                }
                ...
                if (RadComboReservations.Items.Count > 0 && !string.IsNullOrEmpty(RadComboReservations.SelectedValue) && int.Parse(RadComboReservations.SelectedValue) > 0)
                {
                    ...
                    if (totalQuentity > reservationqty)
                    {
                        ...
                        cv.IsValid = false;
                        cv.ValidationGroup = "valGrpOrderHeader";
                        cv.ErrorMessage = "Your order quentity is greater than reservation quentity ";
                        Page.Validators.Add(cv);
                    }
                }
                ...
                if (Page.IsValid)
                {
                    ...
                    //PopulateOrderDetails();
                    //btnCancelOrder_ServerClick(null, null);
            ...
            {
                Order = int.Parse(Session["OrderID"].ToString());
            }
        ...
        private void PopulateControls(bool ispostback)
        {
            try
            {
                if (QueryID == 0 && ispostback == true && VisualLayoutID == -1)
                {
                    liOrderNumber.Visible = false;
                }
                ddlStatus.Enabled = (QueryID > 0) ? true : false;
                ...
                //Requested by Prakash
                //dvBillingDetails.Visible = CompanyExtensions.IsIndicoAdelade(LoggedCompany);

                //populate print Distributor PO
                ...
                if ((OrderID == 0 && VisualLayoutID > 0 && ispostback == true) || (newClient != 0 && OrderID == 0 && ispostback == true)) //||) // (DistributorID > 0 && OrderID == 0 && ispostback == true))
                {
                    // populateOrders if we add new VisualLayout
                    populateOrder();
                }
                else
                {
                    //populate order detail status

                    if (OrderID > 0 || Order > 0)
                    {
                        // ResetDropdowns();
                        PopulateVisualLayouts(0);
                        ...
                        OrderBO objOrder = new OrderBO();
                        objOrder.ID = (OrderID == 0) ? Order : OrderID;
                        objOrder.GetObject();

                        populateLabels(objOrder.Distributor);
                        PopulateDistributorClients(objOrder.Distributor);
                        PopulateJobNames(objOrder.objClient.Client ?? 0);
                        //populateJobNameDetails(objOrder.Client);
                        PopulateMYOBCardNumber(objOrder.Distributor);

                        //Populate Clients
                        //ddlClient.Items.Clear();
                        //ddlClient.Items.Add(new ListItem("Select Client", "0"));
                        //foreach (ClientBO client in objOrder.objDistributor.ClientsWhereThisIsDistributor)
                        //{
                        //    ddlClient.Items.Add(new ListItem(client.Name, client.ID.ToString()));
                        //}

                        //PopulateddlDespatchToCompanyName((int)objOrder.DespatchToExistingClient);

                        //populate Distributor Delivery Address
                        lblDistributorAddress.Visible = true;

                        txtDate.Text = objOrder.Date.ToString("dd MMMM yyyy");
                        txtDesiredDate.Text = (objOrder.EstimatedCompletionDate != null) ? objOrder.EstimatedCompletionDate.ToString("dd MMMM yyyy") : string.Empty;//objOrder.DesiredDeliveryDate.ToString("dd MMMM yyyy");
                        ...
                        //chkPhotoApprovalRequired.Checked = (objOrder.PhotoApprovalReq != null) ? (bool)objOrder.PhotoApprovalReq : false;
                        ddlDistributor.Items.FindByValue(objOrder.Distributor.ToString()).Selected = true;
                        ...
                        //ddlPaymentMethod.Items.FindByValue(objOrder.PaymentMethod.ToString()).Selected = true;
                        //ddlShipmentMode.Items.FindByText("AIR").Selected = false;
                        //ddlShipmentMode.Items.FindByValue(objOrder.ShipmentMode.ToString()).Selected = true;
                        //ddlDeliveryOption.Items.FindByValue((objOrder.DeliveryOption != null && objOrder.DeliveryOption > 0) ? objOrder.DeliveryOption.ToString() : "0").Selected = true;
                ...
                if (LoggedUserRoleName == UserRole.DistributorAdministrator || LoggedUserRoleName == UserRole.DistributorCoordinator)
                {
                    ...
                    //liApprovalRequired.Visible = false;
                    liOrderStatus.Visible = false;
                    //liDespatchto.Visible = false;
                    liClient.Visible = true;
                    //divShipmentMode.Attributes.Add("style", "display:none");
                    //dvWeeklyShipment.Attributes.Add("style", "display:none");
                ...
                if (ispostback && LoggedUser.IsDirectSalesPerson)
                {
                    ...
                    liAccessNo.Visible = false;
                    //liPaymentMethod.Attributes.Add("style", "display:none");
                    // divShipmentMode.Attributes.Add("style", "display:none");
                ...
                //btnPrint.Visible = LoggedUser.IsDirectSalesPerson;
                PopulateOrderStatus(QueryID);
                // ddlVlNumber.Enabled = (!string.IsNullOrEmpty(ddlJobName.SelectedValue) && int.Parse(ddlJobName.SelectedValue) > 0);
                //txtVLSearch.Enabled = !ddlVlNumber.Enabled;
                if (OrderID == 0 && VisualLayoutID > 0)
                {
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    if (isTemp)
                    {
                        ...
                        //int selectedArtWorkID = int.Parse(ddlArtWork.SelectedValue.ToString());
                        int qty = 0;
                        // orderstring = String.Format("Vl NUmber: {0}, OrderType: {1}, Label: {2} ", ddlVlNumber.SelectedValue, ddlOrderType.SelectedValue, ddlLabel.SelectedValue);
                        int orderDetailId = int.Parse(hdnSelectedID.Value);
                        ...
                        if (orderDetailId > 0)
                        {
                            ...
                            //if (int.Parse(ddlVlNumber.SelectedValue) != objOrderDetail.VisualLayout) //Delete existing order quatities
                            //{
                            //    OrderDetailQtyBO objQty = new OrderDetailQtyBO();
                            //    objQty.OrderDetail = objOrderDetail.ID;
                            //    List<OrderDetailQtyBO> lstOrderQtyTmp = objQty.SearchObjects(); // objOrderDetail.OrderDetailQtysWhereThisIsOrderDetail;
                            //    foreach (OrderDetailQtyBO orderQty in lstOrderQtyTmp)
                            //    {
                            //        OrderDetailQtyBO objOrderQtyTmp = new OrderDetailQtyBO(ObjContext);
                            ...
                            if (!string.IsNullOrEmpty(NNFileName) && !string.IsNullOrEmpty(objOrderDetail.NameAndNumbersFilePath))
                            {
                                string oldFilieLocation = IndicoConfiguration.AppConfiguration.PathToDataFolder + "/NameAndNumbersFiles/" + objOrderDetail.Order.ToString() + "/" + objOrderDetail.ID.ToString() + "/" + objOrderDetail.NameAndNumbersFilePath;
                            ...
                            {
                                NNFileName = objOrderDetail.NameAndNumbersFilePath;
                            }
                        }
                        ...
                        foreach (RepeaterItem item in rptSizeQty.Items)
                        {
                            ...
                            OrderDetailQtyBO objOrderDetailQty = new OrderDetailQtyBO(ObjContext);
                            if (objOrderDetail.ID != 0) //Order is in edit mode
                            {
                                if (orderQtyId > 0)
                                {
                            ...
                            qty = qty + int.Parse(txtQty.Text);

                            if (orderQtyId == 0)
                            {
                                objOrderDetail.OrderDetailQtysWhereThisIsOrderDetail.Add(objOrderDetailQty);
                            }
                        }
                        ...
                        if (selectedVisualLayoutID > 0)
                        {
                            VisualLayoutBO objVL = new VisualLayoutBO(ObjContext);
                            objVL.ID = selectedVisualLayoutID;
                            objVL.GetObject();

                            objOrderDetail.VisualLayout = objVL.ID;
                            objOrderDetail.Pattern = objVL.Pattern;
                            objOrderDetail.FabricCode = objVL.FabricCode ?? 0;
                        }
                        else
                        {
                            objOrderDetail.VisualLayout = null;
                            objOrderDetail.Pattern = int.Parse(ddlPattern.SelectedValue.ToString());
                            objOrderDetail.FabricCode = int.Parse(ddlFabric.SelectedValue.ToString());
                        }

                        //else if (selectedArtWorkID > 0)
                        //{
                        //    ArtWorkBO objArtWork = new ArtWorkBO(ObjContext);
                        //    objArtWork.ID = selectedArtWorkID;
                        //    objArtWork.GetObject();

                        //    objOrderDetail.ArtWork = objArtWork.ID;
                        //    objOrderDetail.Pattern = objArtWork.Pattern;
                        //    objOrderDetail.FabricCode = objArtWork.FabricCode;
                        //}

                        objOrderDetail.OrderType = int.Parse(ddlOrderType.SelectedValue);
                        objOrderDetail.Label = int.Parse(ddlLabel.SelectedValue);
                        objOrderDetail.NameAndNumbersFilePath = NNFileName; //File goes to --> /NameAndNumbersFiles/ClientOrderHeaderID/ClientOrderDetailID/FileName

                        objOrderDetail.VisualLayoutNotes = txtOdNotes.Text.ToString();
                        objOrderDetail.PhotoApprovalReq = (chkPhotoApprovalRequired.Checked);
                        objOrderDetail.IsBrandingKit = (chkBrandingKit.Checked);
                        objOrderDetail.IsLockerPatch = (chkLockerPatch.Checked);
                        objOrderDetail.RequestedDate = WeekEndDate;
                        objOrderDetail.SheduledDate = WeekEndDate;
                        objOrderDetail.ShipmentDate = WeekEndDate;
                        objOrderDetail.IsRequiredNamesNumbers = (rbNaNuYes.Checked == true) ? true : (!rbNaNuNo.Checked);
                        try
                        {
                            objOrderDetail.EditedPrice = decimal.Parse(txtDistributorPrice.Text);
                        }
                        catch (Exception)
                        {
                            objOrderDetail.EditedPrice = decimal.Parse("0.00");
                        }

                        objOrderDetail.EditedPriceRemarks = txtPriceComments.Text;
                        objOrderDetail.FactoryInstructions = txtFactoryDescription.Text;
                        objOrderDetail.PromoCode = txtPromoCode.Text;

                        try
                        {
                            objOrderDetail.Surcharge = decimal.Parse(txtSurcharge.Text);
                        }
                        catch (Exception)
                        {
                            objOrderDetail.Surcharge = decimal.Parse("0.00");
                        }
                        ...
                        //objDetail.ArtWork = selectedArtWorkID;
                    ...
                    ViewState["OrderID_" + LoggedUser.ID.ToString()] = objOrder.ID;

                    string NNFileSourceLocetion = IndicoConfiguration.AppConfiguration.PathToDataFolder + "\\Temp\\" + NNFileName;
                    string NNFileDestinationPath = IndicoConfiguration.AppConfiguration.PathToDataFolder + "\\NameAndNumbersFiles\\" + objOrder.ID.ToString() + "\\" + objOrderDetail.ID.ToString();

                    if (NNFileName != string.Empty)
                    {
                        if (File.Exists(NNFileSourceLocetion))
                        {
                ...
                if (isTemp == false)
                {
                    //     SendOrderEmail(OrderID, LoggedCompany.objOwner.EmailAddress, LoggedCompany.objOwner.GivenName + " " + LoggedUser.FamilyName, false, OrderStatus.New);
                }
            }
            catch (Exception ex)
            {
        ...
        private void PopulateOrderDetails()
        {
            ...
            if (OrderID > 0)
            {
                ...
                dgOrderItems.DataSource = lstOrderDetails;
        ...
        private void PopulateVisualLayouts(int client)
        {
            List<VisualLayoutBO> lstVisualLayout = new List<VisualLayoutBO>();
            //List<ArtWorkBO> lstArtWork = new List<ArtWorkBO>();
            VisualLayoutBO objVisualLayout = new VisualLayoutBO();
            //ArtWorkBO objArtWork = new ArtWorkBO();
            ...
            else
            {
                if (OrderID == 0) //&& VisualLayoutID > 0)
                {
                    objVisualLayout.Client = client;
                    //objArtWork.Client = client;
                }
                //else if (OrderID == 0 && ArtWorkID > 0)
                //{

                //}
                else
                {
                    client = int.Parse(ddlJobName.SelectedValue);
                }
            }
        ...
        private void PoplatePatterns()
        {
            ...
            //List<FabricCodeBO> lstFabricCodes = new FabricCodeBO().SearchObjects();
        ...
        private void PopulateReservationsDetails()
        {
            ...
            litReservationDetail.Visible = (id > 0) ? true : false;
            collapse4.Attributes.Add("class", "accordion-body collapse in");
        }
        ...
        private void populateOrder()
        {
            ...
            if (newClient > 0 || objPopulateOrders.Client > 0)
            {
                ...
                //else if (objPopulateOrders.ArtWork > 0)
                //{
                //    ddlArtWork.Items.FindByValue(objPopulateOrders.ArtWork.ToString()).Selected = true;
                //    ddlArtWork_SelectedIndexChanged(null, null);
                //}

                //if (billingAddress > 0)
                //{
                //    objPopulateOrders.BillingAddress = billingAddress;
                //}
                //if (despatchAddress > 0)
                //{
                //    objPopulateOrders.DespatchAddress = despatchAddress;
                //}

                //if (objPopulateOrders.BillingAddress > 0)
                //{
                //    ddlBillingAddress.Items.FindByValue(objPopulateOrders.BillingAddress.ToString()).Selected = true;

                //    DistributorClientAddressBO objDCA = new DistributorClientAddressBO();
                //    objDCA.ID = objPopulateOrders.BillingAddress;
                //    objDCA.GetObject();
                ...
                //if (objPopulateOrders.DespatchAddress > 0)
                //{
                //    ddlDespatchAddress.Items.FindByValue(objPopulateOrders.DespatchAddress.ToString()).Selected = true;

                //    DistributorClientAddressBO objDCA = new DistributorClientAddressBO();
                //    objDCA.ID = objPopulateOrders.DespatchAddress;
                //    objDCA.GetObject();
                ...
                //if (objPopulateOrders.CourierAddress > 0)
                //{
                //    ddlCourierAddress.Items.FindByValue(objPopulateOrders.CourierAddress.ToString()).Selected = true;

                //    DistributorClientAddressBO objDCA = new DistributorClientAddressBO();
                //    objDCA.ID = objPopulateOrders.CourierAddress;
                //    objDCA.GetObject();
