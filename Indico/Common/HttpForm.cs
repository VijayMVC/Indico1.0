    ...
    public class HttpForm
    {
        ...
        private HttpWebResponse UploadFiles(Dictionary<string, string> files, Dictionary<string, string> otherValues)
        {
            ...
            try
            {
                if (otherValues != null)
                {
                    foreach (var fieldName in otherValues.Keys)
                    {
                        ...
                        part.Data = new MemoryStream(Encoding.UTF8.GetBytes(otherValues[fieldName].ToString()));
                ...
                IndicoLogging.log.Error("Successfully finish the Http Post");
            ...
            catch (Exception ex)
            {
                ...
                IndicoLogging.log.Error("Error occured while sending the data from the HttpForm ", ex);
