...
(function ($) {
    ...
    UploadHandler = function (container, options) {
        ...
        this.initUploadRow = function (event, files, index, xhr, handler, callBack) {
            ...
            var complete = false;
            var multirow = 'true';
            for (var i = 0; i < handler.uploadTable[0].attributes.length; i++) {
                if (handler.uploadTable[0].attributes[i].nodeName == 'multirow') {
                    multirow = handler.uploadTable[0].attributes[i].nodeValue;
                    if (multirow === 'false') {
                        while (handler.uploadTable[0].rows.length > 0) {
                            handler.uploadTable[0].deleteRow(0);
                        }
                        if (complete) {
                            break;
                        }
                ...
                else if (handler.uploadTable[0].attributes[i].nodeName == 'setdefault') {
                    var setdefault = handler.uploadTable[0].attributes[i].nodeValue;
                    if (setdefault === 'true') {
                        handler.uploadTable.prev()[0].style.display = 'none';
                        if (complete) {
                            break;
                        }
                        else {
            ...
            var file = files[index].name.replace(/C:\\/g, 'c:\\').replace(/c:\\fakepath\\/g, '');
            var name = file.substring(0, file.lastIndexOf("."));
            var extension = file.substring(file.lastIndexOf("."), file.length);            
            var serverFileName = name.toLowerCase().replace(/ /g, '-') + '_' + key + extension;
            if (multirow === 'true') {
                $(document.getElementById(handler.uploadTable[0].id)).next()[0].value += serverFileName + "," + file + "|";
            }
            else {
                $(document.getElementById(handler.uploadTable[0].id)).next()[0].value = serverFileName + "," + file;
            }
